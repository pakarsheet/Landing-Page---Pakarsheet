...
export default async function AdminCustomOrdersPage() {
  ...
  return (
    <div className="space-y-7">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-ink">Custom Orders</h1>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="flex items-center gap-2.5 rounded-xl border border-line bg-white px-5 py-3 shadow-sm">
          ...
          <span className="text-base font-semibold text-ink">{baruCount}</span>
          <span className="text-sm text-muted">Baru</span>
        </div>
        <div className="flex items-center gap-2.5 rounded-xl border border-line bg-white px-5 py-3 shadow-sm">
          <MessageCircle className="h-4 w-4 text-muted" />
          <span className="text-base font-semibold text-ink">{dihubungiCount}</span>
          <span className="text-sm text-muted">Dihubungi</span>
        </div>
        <div className="flex items-center gap-2.5 rounded-xl border border-line bg-white px-5 py-3 shadow-sm">
          <Clock className="h-4 w-4 text-muted" />
          <span className="text-base font-semibold text-ink">{negosiasiCount}</span>
          <span className="text-sm text-muted">Negosiasi</span>
        </div>
        <div className="flex items-center gap-2.5 rounded-xl border border-line bg-white px-5 py-3 shadow-sm">
          <Handshake className="h-4 w-4 text-muted" />
          <span className="text-base font-semibold text-ink">{dealCount}</span>
      ...
      {baruCount > 0 && (
        <div className="flex items-center gap-3 rounded-2xl border border-cobalt/20 bg-sky/30 px-5 py-3.5">
          <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-cobalt text-white text-sm font-bold">
            {baruCount}
          </span>
