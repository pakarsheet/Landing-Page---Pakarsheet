  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-line bg-white shadow-sm">
      ...
      <div className="flex items-center justify-between border-b border-line px-6 py-5">
        ...
        <Link
          href="/admin/products"
          className="flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-sm font-semibold text-cobalt transition hover:bg-blush"
        >
          Lihat semua
      ...
      {!recentProducts?.length ? (
        <div className="py-16 text-center text-sm text-muted">Belum ada produk</div>
      ) : (
        <ul className="divide-y divide-line/60">
          {recentProducts.map((p) => (
            <li
              key={p.id}
              className="flex items-center justify-between px-6 py-4 transition hover:bg-[#f4f6fb]"
            >
              <div className="flex items-center gap-4 min-w-0">
                <span
                  className={`h-2.5 w-2.5 shrink-0 rounded-full ${
                    p.status === "active" ? "bg-sheet" : "bg-line"
                  }`}
                />
                <div className="min-w-0">
                  <p className="truncate text-sm font-semibold text-ink">{p.title}</p>
                  <p className="mt-0.5 text-xs text-muted">{p.category} · {p.price}</p>
                </div>
              </div>
              <div className="ml-4 flex shrink-0 items-center gap-3">
                ...
                <Link
                  ...
                  className="flex h-7 w-7 items-center justify-center rounded-lg text-sm text-muted transition hover:bg-ink/8 hover:text-ink"
                >
                  →
                </Link>
              </div>
