...
export function WorksheetCard({ worksheet }: Props) {
  const Icon = worksheet.icon;

  return (
    <Link
      href={`/tools/${worksheet.slug}`}
      className="worksheet-card group flex flex-col rounded-3xl border border-line bg-white shadow-card transition duration-300 hover:-translate-y-1 hover:shadow-soft"
    >
      <div className="flex flex-1 flex-col p-6">
        {/* Header */}
        <div className="flex items-start justify-between gap-3">
          <span className={`flex h-11 w-11 items-center justify-center rounded-2xl ${worksheet.accent}`}>
          ...
          <div className="flex flex-col items-end gap-1.5">
            ...
            {worksheet.estimatedTime && (
              <span className="inline-flex items-center gap-1 font-secondary text-[10px] font-semibold text-muted">
